...
// clang-format off
#define KY_PROFILE_RENDER_CREATE(renderer) ky_profile_render_create(renderer)
#define KY_PROFILE_RENDER_ZONE_COLOR(renderer, var, name, color) do { static const struct ___tracy_source_location_data var = { name, __func__, __FILE__, __LINE__, color }; ky_profile_render_begin(renderer, &var); } while (0)
#define KY_PROFILE_RENDER_ZONE(renderer, var, name) KY_PROFILE_RENDER_ZONE_COLOR(renderer, var, name, 0x8b0000)
#define KY_PROFILE_RENDER_ZONE_END(renderer) ky_profile_render_end(renderer)
#define KY_PROFILE_RENDER_COLLECT(renderer) ky_profile_render_collect(renderer)
// clang-format on
