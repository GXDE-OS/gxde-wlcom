...
static void xdg_view_configure(struct view *view)
{
    ...
    wlr_xdg_toplevel_set_tiled(wlr_xdg_toplevel, kywc_view->activated);
    wlr_xdg_toplevel_set_tiled(wlr_xdg_toplevel, kywc_view->resizing);
...
{
    struct wlroots_server *wlroots = wl_container_of(listener, wlroots, new_xdg_surface);
...
{
    struct wlroots_server *wlroots = wl_container_of(listener, wlroots, new_xwayland_surface);
}
#endif

bool wlroots_shell_init(struct wlroots_server *wlroots)
{
    struct wlr_xdg_shell *xdg_shell = wlr_xdg_shell_create(wlroots->server->display, 5);

    wlroots->new_xdg_surface.notify = handle_new_xdg_surface;
    wl_signal_add(&xdg_shell->events.new_surface, &wlroots->new_xdg_surface);

#if HAVE_XWAYLAND
    if (wlroots->xwayland) {
        wlroots->new_xwayland_surface.notify = handle_new_xwayland_surface;
        wl_signal_add(&wlroots->xwayland->events.new_surface, &wlroots->new_xwayland_surface);
    }
#endif
