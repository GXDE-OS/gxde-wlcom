...
static float animation_value(struct animation *animation, float x)
{
...
static struct animation *animation_create(float p1x, float p1y, float p2x, float p2y)
{
...
struct animator *animator_create(struct animation_data *start, int64_t start_time, int64_t end_time)
{
    struct animator *animator = calloc(1, sizeof(*animator));
    if (!animator) {
        return NULL;
    }
    animator->animation = animation_create(0, 0, 0.22, 1);
...
{
    animation_destroy(animator->animation);
    free(animator);
}
...
void animator_set_angle(struct animator *animator, float end_angle)
{
    ...
    linear_function_init(&animator->angle_func, animator->animation_value, animator->current.angle,
...
void animator_set_alpha(struct animator *animator, float end_alpha)
{
    ...
    linear_function_init(&animator->alpha_func, animator->animation_value, animator->current.alpha,
...
void animator_set_position(struct animator *animator, int end_x, int end_y)
{
    if (end_x != animator->end.geometry.x) {
        ...
        linear_function_init(&animator->geometry_func[0], animator->animation_value,
    ...
    if (end_y != animator->end.geometry.y) {
        ...
        linear_function_init(&animator->geometry_func[1], animator->animation_value,
...
void animator_set_size(struct animator *animator, int end_width, int end_height)
{
    if (end_width != animator->end.geometry.width) {
        ...
        linear_function_init(&animator->geometry_func[2], animator->animation_value,
    ...
    if (end_height != animator->end.geometry.height) {
        ...
        linear_function_init(&animator->geometry_func[3], animator->animation_value,
...
const struct animation_data *animator_value(struct animator *animator, int64_t current_time)
{
    ...
    if (animator->masks & ANIMATOR_ALPHA) {
        animator->current.alpha = value * animator->alpha_func.k + animator->alpha_func.b;
    }
    if (animator->masks & ANIMATOR_GEOMETRY_X) {
        animator->current.geometry.x =
            value * animator->geometry_func[0].k + animator->geometry_func[0].b;
    }
    if (animator->masks & ANIMATOR_GEOMETRY_Y) {
        animator->current.geometry.y =
            value * (animator->geometry_func[1].k) + animator->geometry_func[1].b;
    }
    if (animator->masks & ANIMATOR_GEOMETRY_W) {
        animator->current.geometry.width =
            value * animator->geometry_func[2].k + animator->geometry_func[2].b;
    }
    if (animator->masks & ANIMATOR_GEOMETRY_H) {
        animator->current.geometry.height =
            value * animator->geometry_func[3].k + animator->geometry_func[3].b;
    }
