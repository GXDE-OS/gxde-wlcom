...
/*
 * Functions that run the algorithm on the provided data and put the digest into hash.
 * result of hash should be able to store 16 bytes.
 */
void md5_generate(const void *data, size_t length, uint8_t *hash)
{
    MD5Context ctx;
    md5Init(&ctx);
    md5Update(&ctx, (uint8_t *)data, length);
    md5Finalize(&ctx);

    memcpy(hash, ctx.digest, MD5_DIGEST_LENGTH);
}
...
char *md5_to_string(uint8_t *md5, size_t len, char *buf, size_t buf_size)
{
    ...
    for (size_t i = 0; i < len; i++) {
        snprintf(buf + i * 2, buf_size - i * 2, "%02x", md5[i]);
    }
    buf[len * 2] = '\0';
    return buf;
}
